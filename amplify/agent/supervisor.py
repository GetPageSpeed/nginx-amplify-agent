...
def log_cloud_messages(messages, log):
    """Log admin broadcast messages returned in a CloudResponse.

    Args:
        messages: List of message strings from cloud_response.messages.
        log: Logger to write each non-empty message to (INFO level).
    """
    for msg in messages:
        if msg:
            log.info("amplify message: %s", msg)
...
class Supervisor:
    ...
    def init_object_managers(self):
        ...
        for object_type in self.object_manager_order:
            try:
                ...
                self.object_managers[object_type] = manager_class(object_configs=object_configs)
                context.log.debug(f'loaded "{object_type}" object manager from {manager_class}')
            except Exception:
                context.log.error(f"failed to load {object_type} object manager", exc_info=True)
    ...
    def load_ext_managers(self):
        ...
        from amplify.ext.abstract.manager import AbstractExtManager, ExtObjectManager
        ...
        def _recursive_manager_init(inspected_package, prefix=base_prefix, top_mod=None):
            ...
            for _, modname, ispkg in pkgutil.iter_modules(inspected_package.__path__):
                ...
                if not enabled_extension(top_mod):
                    context.log.debug(f'ignored "{top_mod}" module during ext scan')
                    continue
                ...
                if ispkg:
                    ...
                    recursive_mod_path = _recursive_manager_init(mod, prefix=current_prefix, top_mod=top_mod)
        ...
        while True:
            ...
            try:
                ...
                external_object_managers = filter(
                    lambda x: x not in self.object_manager_order, self.object_managers.keys()
                )
                ...
                if e.errno == 12:  # OSError errno 12 is a memory error (unable to allocate, out of memory, etc.)
                    context.log.error(f"OSError: [Errno {e.errno}] {e.message}", exc_info=True)
                    continue
                else:
                    raise e
    ...
    def stop_everything(self):
        ...
        # stop other managers
        ext_managers = filter(lambda name: name not in self.object_manager_order, self.object_managers.keys())
        for object_manager_name in ext_managers:
            object_manager = self.object_managers[object_manager_name]
            object_manager.stop()

        # log agent stopped event
        context.log.info(f"agent stopped, version={context.version} pid={context.pid} uuid={context.uuid}")
    ...
    def talk_to_cloud(self, root_object=None, force=False, initial=False):
        ...
        now = int(time.time())
        if not force and (
            now <= (self.last_cloud_talk_time + context.app_config["cloud"]["talk_interval"] + self.cloud_talk_delay)
        ...
        try:
            ...
            cloud_response = CloudResponse(context.http_client.post("agent/", data=root_object))
            ...
            if isinstance(e, HTTPError) and e.response.status_code == 503:
                ...
                context.log.debug(f"cloud talk delay set to {self.cloud_talk_delay} (fails: {self.cloud_talk_fails})")

            context.log.error("could not connect to cloud", exc_info=True)
            raise AmplifyCriticalException()

        log_cloud_messages(cloud_response.messages, context.log)

        # check agent version status
        if context.version_semver <= cloud_response.versions.obsolete:
            context.log.error(
                f"agent is obsolete - cloud will refuse updates until it is updated (version: {tuple_to_version(context.version_semver)}, current: {tuple_to_version(cloud_response.versions.current)})"
            )
            self.stop()
        elif context.version_semver <= cloud_response.versions.old:
            context.log.warn(
                f"agent is old - update is recommended (version: {tuple_to_version(context.version_semver)}, current: {tuple_to_version(cloud_response.versions.current)})"
            )
        ...
        for obj in cloud_response.objects:
            ...
            if object_manager.object_configs.get(obj.id, {}) != obj.config:
                context.log.info(
                    f'object config has changed. now "{obj.type}" {obj.id} is running with: {pprint.pformat(obj.config)}'
                )
        ...
        for object_type, object_manager in self.object_managers.items():
            for obj_id in object_manager.object_configs.keys():
                if obj_id not in matched_object_configs:
                    context.log.debug(
                        f'object config has changed. now "{object_type}" {obj_id} is running with default settings'
                    )
        ...
        def _recursive_dict_match_only_existing(kwargs1, kwargs2):
            for k, v1 in kwargs1.items():
                if isinstance(v1, dict):
                    ...
                    if not _recursive_dict_match_only_existing(v1, kwargs2.get(k, {})):
                        return False
                else:
        ...
        config_changed = not _recursive_dict_match_only_existing(cloud_response.config, context.app_config.default)
        ...
        if config_changed or len(changed_object_managers) > 0:
            ...
            if config_changed:
                context.log.debug(
                    f"app config has changed. now running with: {pprint.pformat(context.app_config.config)}"
                )
                ...
                if self.object_managers:
                    ...
                    for name in self.external_managers.keys():
                        attr_string = f"{name}_manager"
                        thread = getattr(self, attr_string, None)
                        if thread is not None:
                            thread.kill()

            elif len(changed_object_managers) > 0:
                context.log.debug(f"obj configs changed. changed managers: {list(changed_object_managers)}")
    ...
    def check_bridge(self):
        ...
        if self.bridge.ready and self.bridge.exception:
            context.log.debug(f"bridge exception: {self.bridge.exception}")
    ...
    def manage_external_managers(self):
        ...
        for name, manager_cls in self.external_managers.items():
            ...
            if thread is None:
                ...
                context.log.debug(f'starting "{manager_cls.__name__}" external manager')
                setattr(self, attr_string, gevent.spawn(manager_cls().start))
            elif thread.dead:
                # manager was stopped (or thread killed)
                context.log.debug(f'starting "{manager_cls.__name__}" external manager after stop')
                setattr(self, attr_string, gevent.spawn(manager_cls().start))
            elif thread.ready and thread.exception:
                context.log.debug(f'restarting "{manager_cls.__name__}" external manager')
